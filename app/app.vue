...
const todasAsTrilhas = [
  ...
  { id: 3, nome: "Trilha 3: Empreendedorismo e Inovação", status: "Em breve", icon: "💡", aulas: [] }
];
// 4. Estados Reativos de Navegação e Dados
const step = ref('trilha-selection');
const selectedTrilha = ref(null);
const currentQuiz = ref(null);
const userData = ref({ nome: '', municipio: '', cpf: '' });
...
const startQuiz = () => {
  // Validação do CPF (que já fizemos)
  if (!validarCPF(userData.value.cpf)) {
    cpfError.value = "CPF inválido.";
    return;
  }

  // Validação do Município
  const cidadeValida = municipiosCeara.find(
    m => m.toLowerCase() === userData.value.municipio.toLowerCase().trim()
  );

  if (!cidadeValida) {
    alert("Por favor, selecione um município válido do Ceará na lista.");
    return;
  }

  // Se for válida, usamos o nome com a acentuação correta da lista
  userData.value.municipio = cidadeValida;
...
const finishAndSave = async () => {
  ...
  try {
      ...
      .insert([
        {
          ...
          created_at: agora // Opcional, o Supabase já faz isso via default now()
        }
      ]);
    ...
    alert("Erro ao registar no banco de dados. Mas podes ver o teu resultado!");
    step.value = 'results';
  }
};
// Estados adicionais
...
src="https://cearadevalores.com.br/wp-content/uploads/2025/09/cropped-estrela-e1756834379223-32x32.png"
        ...
        <button v-if="step !== 'trilha-selection'" class="text-xs bg-blue-800 px-3 py-1 rounded-lg" @click="goBack">
          ← Voltar
        </button>
      </div>
    </header>
...
v-for="aula in selectedTrilha.aulas" :key="aula.id"
      ...
      <div v-else-if="step === 'register'" class="bg-white p-8 rounded-3xl shadow-2xl border border-blue-50">
        <div class="text-center mb-6">
          <span class="bg-orange-100 text-orange-600 px-4 py-1 rounded-full text-xs font-black uppercase">Falta
            pouco!</span>
        ...
        <div class="space-y-4">
          <div class="space-y-1">
            <label class="text-xs font-black text-slate-400 uppercase ml-1">Seu Nome Completo</label>
            <input
v-model="userData.nome" type="text"
              class="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:border-blue-500 outline-none transition-all"
              placeholder="Ex: João da Silva">
          </div>
          <div class="space-y-1 relative">
            <label class="text-xs font-black text-slate-400 uppercase ml-1">Seu Município</label>

            <input
v-model="userData.municipio" type="text"
              class="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:border-blue-500 outline-none transition-all"
              placeholder="Ex: Iguatu, Sobral, Crato..." @focus="mostrarSugestoes = true"
              @blur="setTimeout(() => mostrarSugestoes = false, 200)">

            <transition name="fade">
              <div
v-if="mostrarSugestoes && sugestoesFiltradas.length > 0"
                class="absolute z-10 w-full mt-1 bg-white border border-slate-100 shadow-xl rounded-2xl overflow-hidden">
                <button
v-for="cidade in sugestoesFiltradas" :key="cidade"
                  class="w-full p-4 text-left hover:bg-blue-50 border-b border-slate-50 last:border-0 transition-colors flex justify-between items-center"
                  @click="selecionarMunicipio(cidade)">
                  <span class="font-bold text-slate-700">{{ cidade }}</span>
                  <span class="text-xs text-blue-400 font-bold">Selecionar</span>
                </button>
              </div>
            </transition>

            <p
v-if="userData.municipio && !municipiosCeara.includes(userData.municipio) && userData.municipio.length > 3"
              class="text-[10px] text-orange-500 font-bold ml-1 italic">
...
v-if="jaRespondeu"
          ...
          <div class="space-y-1 relative">
            ...
            <div class="relative">
              <input
v-model="userData.cpf" type="text" maxlength="14"
                class="w-full p-4 bg-slate-50 border-2 rounded-2xl outline-none transition-all" :class="[
                  cpfError ? 'border-red-300 focus:border-red-500' : 'border-slate-100 focus:border-blue-500',
                  userData.cpf?.length === 14 && !cpfError ? 'border-green-400' : ''
          ...
          <button
            class="w-full bg-blue-600 text-white font-black py-4 rounded-2xl shadow-lg shadow-blue-200 hover:bg-blue-700 active:scale-95 transition-all"
            @click="startQuiz">
            COMEÇAR REVISÃO
          </button>
        </div>
...
v-if="isAnswered"
            ...
            <p class="text-[10px] font-black text-blue-600 uppercase mb-1 tracking-widest">💡 Por que esta resposta?</p>
